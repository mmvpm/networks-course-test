...
запрос. В таком случае вам необходимо указывать в заголовке запроса значение для If-ModifiedSince и If-None-Match. 
...
1. Что такое база данных whois?
   - <!-- todo -->
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-северов. 
   Какие сервисы вы при этом использовали?
   - <!-- todo -->
   - <!-- todo -->
3. Используйте команду nslookup на локальном хосте, чтобы послать запросы трем серверам
   DNS: вашему локальному серверу DNS и двум DNS-серверам, найденным в предыдущей части.
   - <!-- todo -->
