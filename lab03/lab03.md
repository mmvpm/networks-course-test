...
работу при помощи отправки запросов через браузер.
...
порт сервера и имя файла на сервере. Формат команды для запуска клиента следующий:
```
client.exe хост_сервера порт_сервера имя_файла
```

#### Скрины
Продемонстрируйте работу клиента.
<!-- todo -->
...
сервер считывает целочисленное значение `concurrencyLevel` (из командной строки, из
конфигурационного файла или еще каким-либо образом). Если сервер получает запрос от клиента,
и при этом уже запущено максимальное количество потоков, то запрос от клиента блокируется
(встает в очередь) и дожидается, пока не закончит работу один из запущенных потоков. После
этого сервер может запустить новый поток для обработки запроса от клиента.
...
битов, имеющий скорость $128$ Кбит/c, и разбивает его на $56$-байтные пакеты. Хосты А и Б
соединены одной линией связи, в которой скорость передачи данных равна $1$ Мбит/c, а задержка
...
ожидания и передачи), испытываемую пакетом. Согласно формуле Литтла $N = а \cdot d$.
...
является первая линия связи, скорость передачи данных по которой равна $R_S$ бит/c.
